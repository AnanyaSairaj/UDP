 ...
 /* ---------------------------------PUT CASE-----------------------------*/
  ...
  if(f>0)
    {  sendto(sock, &(total_frame),sizeof(total_frame), 0, (struct sockaddr *) &client, sizeof(client));
      ...
      sendto(sock, &(frame.ID), sizeof(frame.ID), 0, (struct sockaddr *) &client, (socklen_t *) &length);
...
/*------------------------------GET CASE-----------------------------*/
  ...
  if ((strcmp(cmdreceive,"get")==0)&& (strcmp(filenamereceive,"")!=0))
 
  {
  
  printf("SERVER: To GET file named %s",filenamereceive);
